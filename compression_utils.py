...
def strip_compression_suffix(name: str) -> Tuple[str, Optional[str]]:
...
def content_encoding_to_suffix(value: Optional[str]) -> Optional[str]:
    if value is None:
        return None
    return CONTENT_ENCODING_TO_SUFFIX.get(value.strip().lower())


def suffix_to_content_encoding(suffix: Optional[str]) -> Optional[str]:
    if suffix is None:
        return None
